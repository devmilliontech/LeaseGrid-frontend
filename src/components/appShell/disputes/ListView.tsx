...
import { Calendar, EyeIcon, Folder, MessageSquare, Star, Mail, Phone, Flag, FileText, ImageIcon, Play, Clock10, SendHorizonal, CircleCheck, X, CircleX, } from "lucide-react";
...
export const ListView: React.FC<QueueViewProps> = ({ data }) => {
    ...
    const rowlist = "flex flex-row justify-center w-full items-center"
    return (
        <div>
            <div className=" px-4 py-2 rounded-2xl bg-white w-full">
                ...
                <div className={`${rowlist} border-b pb-5 border-slate-200`}>
                    {tableHeaders.map((header, index) => {
                        return (
                            <p key={index} className={`${subject} ${rowlist} `}>{header}</p>
                        )
                    })}
                </div>
                <div className="flex flex-col overflow-y-auto scrollbar-hide h-120 " >
                    {data.map((item, index) => {
                        return (
                            <div
                                key={index}
                                className={`bg-slate-50 hover:bg-slate-100 ${rowlist} rounded-2xl py-3 mt-2`}
                            >
                                <div className={`${rowlist}`}>
                                    <p className="text-sm font-semibold text-teal-500">#{item.disputeId}</p>
                                </div>
                                <div className={`${rowlist} gap-2 `}>
                                    <UserAvatar img={item.complainant.avatar} className={`border-1 border-red-100 w-10 h-10 `} />
                                    <p className={subSubject}>vs</p>
                                    <UserAvatar img={item.respondent.avatar} className={`border-1 border-red-100 w-10 h-10 `} />
                                </div>
                                <div className={`${rowlist}`}>
                                    <p className={`${subSubject} whitespace-nowrap`}>{item.issue}</p>
                                </div>
                                <div className={`${rowlist}`}>
                                    <p className={`text-xs font-medium px-3 py-1 rounded-full ${statusColor(item.status)}`}>{item.status}</p>
                                </div>
                                <div className={`${rowlist} `}>
                                    <p className={`${subSubSubject} whitespace-nowrap`}>{GetDays(item.date, item.time)}</p>
                                </div>
                                <div className={`${rowlist} `}>
            ...
            {selectedDispute && open && (
                <Dialog
                    open={open}
                    onClose={() => { setSelectedDispute(null); setOpen(false); }}
                    PaperProps={{
                        sx: {
                            borderRadius: 4,
                            maxWidth: "80%",
                ...
                >
                    <Box>
                        <div className="mt-4 px-4 pt-4">
                            {/* Dispute Header */}
                            <div className="flex flex-row justify-between items-center border-b-2 border-slate-200 pb-6 px-4">
                                ...
                                <div className=" w-full flex justify-end ">
                                    <X className={`w-5 h-5 rounded-full cursor-pointer text-slate-600`}
                                        onClick={() => { setSelectedDispute(null); setOpen(false); }}
                                    />
                                </div>
                            </div>
                            <div className="flex flex-col gap-4 px-4 py-4 overflow-y-auto scrollbar-hide h-[calc(100vh-200px)] ">
                                {/* Parties Involved */}
                                ...
                                <div className="bg-blue-50 border-2 border-blue-200 p-4 rounded-2xl flex flex-col">
